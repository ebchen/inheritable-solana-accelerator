...
const HASH_CONTRACT_ADDRESS = useSolana
  ? "3uCfjcPxnvWyNRSpBQKcDwpBmuAaXraPw8v7SzKicfmq"
  : "0xB46459Cf87f1D6dDcf8AABDd5642cf27a39CeC68";
...
class Verify extends Component {
  ...
  async readClaim() {
    ...
    if (res.data && res.data.length > 0) {
      ...
      this.setState({
        ...
        receivedQty: (item.amount + "").includes(".")
          ? item.amount
          : item.amount + ".0",
      });
    }
  ...
  async check() {
    const { claim, qty, salt } = this.state;
    ...
    try {
      if (success) {
        ...
        if (useSolana) {
          ...
          const programAccount = new solanaWeb3.PublicKey(
          ...
          const mlen =
            ...
            (mlenarr[2] << 16) +
            (mlenarr[3] << 24);
          const mdata = new TextDecoder().decode(
            data.slice(8 + hlen, data.length)
          );
          //console.log(mdata);
        ...
        if (!success) {
          ...
          const resVerifySig = await nodeApi.verifyDataSignature(
            session,
            sigKey,
            chainRootHashSignature,
            toSign
          );
          ...
          if (success) {
            message +=
              "On-chain merkle root hash matching Dilithium signature\n";
            ...
            const resVerifyTree = await nodeApi.verifyMerkleHash(
              session,
              tree,
              recordHash,
              digest
            ...
            claimHash =
              recordHash.substr(0, 5) +
              "..." +
              recordHash.substr(recordHash.length - 5);
        ...
        <div class="text-sm items-center mt-6 px-24 grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-16">
          <div className="col-span-1">
            <div className="flex justify-between">
              <p className="text-zinc-500 font-bold text-2xl text-left">
                Connected Address:{" "}
              </p>
            ...
            <br />
            <div className="flex justify-between">
              <span className="text-zinc-500 font-bold text-2xl text-left">
                Weavechain public key:{" "}
              </span>
              <span className="text-zinc-300 text-lg ml-2">
            ...
            <div class="">
              <div className="rounded-md text-gray pt-4 flex flex-col items-start">
                <p className="text-l py-2 font-semibold">
                  Input Claim Description
                </p>
                <Form
                  styling="w-full h-8 bg-zinc-400"
                  field={claim}
                  onChangeFunc={(event) =>
          ...
          <div>
            ...
            {success ? (
              <>
                <span className="text-green-700 text-center text-lg font-bold">
                  CLAIM VERIFIED
                </span>
              </>
            ) : null}
            {error ? (
              <>
                <span className="text-red-700 text-center text-lg font-bold">
                  {error}
                </span>
              </>
            ) : null}
